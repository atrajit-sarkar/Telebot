...
ents@bot.message_handler(content_types=['document', 'photo'])
def handle_docs_photos(message):
    ...
    try:
        ...
        if f"{dir}/{original_file_name}".endswith(".zip"):
            with zipfile.ZipFile(f"{dir}/{original_file_name}", 'r') as zip_ref:
                os.mkdir(f"{dir}/extracted")
                zip_ref.extractall(f"{dir}/extracted")  
        # time.sleep(0.01)
        bot.send_message(message.chat.id,"Enter the extension to check:")
        bot.register_next_step_handler(message)
        s=0
        for i in os.listdir(f"{dir}/extracted"):
            s=checkfileEXT(f"{dir}/extracted",message)+s
        bot.reply_to(message,f"There are {s} {message} files...")
